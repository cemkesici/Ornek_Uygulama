...
void tarih::tarihh()//Tarih secmek icin gerekli sartlari burda kontrol ettim
{
    ...
    do
    {   s = 0; // s degiskenini sıfıra esitledim
        ...
        if (s == 0)// Eger gun hatali ise ay sormamasi için s 0'a esitse gir diye kosulu yazdım
        {
...
public:
    ...
    void satiskayitsayi();//satis da kac kayit oldugunu bulmak iÃ§in kurulan fonksiyonu tanimladim
...
void satis::satisekle()
{
    ...
    {           s_s = 0;//sayaci sifira esitledik
               ...
               do
               {
                   ...
                   siduzun = surunaranan.length();//girilen degerin uzunlugunu bulup degiskene atadık
                   if (siduzun == 6)
...
void satis::satiskayitsayi() {
    ...
    while (!satisekle.eof())// satisekle dosyasi sona gelene kadar dongunun saglanmasınu sagladik
    {
        getline(satisekle, satissayi);// getline ile satisekleyi satissayi stringine aktardik
        satissayac++;//sayaci 1 arttirdik

    }
    satisno = satissayac - 1;// satissayac degerini bos deger de eklendigi icin 1 cıkartıp satisno ya atadik
...
void satis::satislistele()
{
    ...
    while (!satisekle.eof())// satisekle dosyasi sona gelene kadar dongunun saglanmasınu sagladik
    {
        ...
        {// Eger satisno sifirsa yani sifirinci satirda ise altta olanlari yazirdik
            cout << "No" << "\t" << "İsim" << "\t\t" << "Soy İsim" << "\t\t\t" << "Alinan UrunİD" << "\t\t" << "GÜN" << "." << "AY" << "." << "YİL" << endl;
        }
...
void satis::satissil() {
    do
    {
        ...
        cout << "Silinecek Satis İcin Musterinin Adini Giriniz :"; cin >> silsatis;
        //Ekrana yazdirdik ve deger aldik
        while (!satisekle.eof())// satisekle dosyasini sonuna kadar calistiracak
        {
            satisekle >> smusteriad >> smusterisoyad >> satisadaranan;// satisekle dosyasından verileri aldık
        ...
        rename("sgecici.tmp", "sat.txt");// sgecici.tmp adını sat.txt olarak degistirdik
...
void satis::satisduzenle()
{
    ...
    do
    {
        ...
        cout << "Duzenlenecek Satis İcin Musterinin Adini Giriniz :"; cin >> sildsatis;// ekrana yazdirdik ve girilen degeri atadik
        while (!satisekle.eof())// satisekle dosyasi sona gelene kadar dongunun saglanmasini sagladik
        {
            ...
            if (getsmusteriad() == sildsatis && s_ad == 0)// satisekleden okunan smusteriad degerini girilen degerle ayni mi ve a=0 mi diye kosula soktuk."Get kullandik"
            {
                ...
                //girilen degerleri sdyedek dosyasına girdik
                cout << "Urun eklendi..." << endl;
                //ekrana yazdirdik
                musteriekle << getsmusteriad() << "\t\t" << getsmusterisoyad() << "\t\t\t" << musterinumara << endl;
                //girilen degerleri musteriekle dosyasına girdik
                cout << "Musteri eklendi..." << endl;
                //ekrana yazdirdik
        ...
        rename("sdgecici.tmp", "sat.txt");// sdgecici.tmp adÄ±nÄ± sat.txt yaptik
...
void urunn::urunlistele()
{
    ...
    while (!urunekle.eof())
    {
        if (urunno == 0) 
        {
         cout << "No" << "\t" << "UrunİD" << "\t\t" <<"İsim" << "\t\t" <<"Fiyat" << "\t\t\t" << "Barkod"<< "\t\t" << "GÜN" << "." << "AY" << "." << "YİL" << endl;
        }
        urunno++;
...
void musteri::musterilistele()
{
    ...
    while (!musteriekle.eof())
    {
        if (musterino == 0)
        {
            cout << "No" << "\t" << "İsim" << "\t\t" << "Soy İsim" << "\t\t" << "Tel No." << "\t\t\t" << "GÜN" << "." << "AY" << "." << "YİL" << endl;
        }
        musterino++;
...
{   system("color 80");//rengi ayarladik
    setlocale(LC_ALL, "turkish");//Turkce karakterler okumasını sadladik
